...
List merge(List list1,List list2){
	...
	p = list1.pHead;
	q = list2.pHead;
	while(p!=EndPosition(list1)&&q!=EndPosition(list2)){
		if(p->next->data<q->next->data){
			InsertBefore(&dest,EndPosition(dest),p->next->data);
		    p = p->next;
		}else{
			InsertBefore(&dest,EndPosition(dest),q->next->data);
		    q = q->next;
		}
	}
	
	while(p!=EndPosition(list1)){
		InsertBefore(&dest,EndPosition(dest),p->next->data);
		p = p->next;
	}
	while(q!=EndPosition(list2)){
		InsertBefore(&dest,EndPosition(dest),q->next->data);
		    q = q->next;
	}
	return dest;
